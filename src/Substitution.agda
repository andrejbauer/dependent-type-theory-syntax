...
module Substitution where
  ...
  module Core {𝕊 : SymbolSignature} where
    ...
    _ʳ∘ˢ_ : ∀ {𝕄} {γ δ η} → (𝕊 % δ →ʳ η) → (𝕄 ∥ γ →ˢ δ) → (𝕄 ∥ γ →ˢ η)
    (ρ ʳ∘ˢ σ) x = [ ρ ]ʳ σ x

    infixl 7 _ˢ∘ʳ_

    _ˢ∘ʳ_ : ∀ {𝕄} {γ δ η} → (𝕄 ∥ δ →ˢ η) → (𝕊 % γ →ʳ δ) → (𝕄 ∥ γ →ˢ η)
    (σ ˢ∘ʳ ρ) x =  σ (ρ x)

    -- extension commutes with the composition of renaming and substitution

    ⇑ˢ-ˢ∘ʳ : ∀ {𝕄} {β γ δ η} {ρ : 𝕊 % β →ʳ γ} {σ : 𝕄 ∥ γ →ˢ δ} → ⇑ˢ {η = η} (σ ˢ∘ʳ ρ) ≈ˢ ⇑ˢ σ ˢ∘ʳ (𝕊 %⇑ʳ ρ)
    ⇑ˢ-ˢ∘ʳ (var-left x) = ≈-refl
    ⇑ˢ-ˢ∘ʳ (var-right y) = ≈-refl

    ⇑ˢ-ʳ∘ˢ : ∀ {𝕄} {β γ δ η} {σ : 𝕄 ∥ β →ˢ γ} {ρ : 𝕊 % γ →ʳ δ} → ⇑ˢ {η = η} (ρ ʳ∘ˢ σ) ≈ˢ (𝕊 %⇑ʳ ρ) ʳ∘ˢ ⇑ˢ σ
    ...
    [ˢ∘ʳ]ˢ : ∀ {𝕄 cl} {γ δ η} → {σ : 𝕄 ∥ δ →ˢ η} → {ρ : 𝕊 % γ →ʳ δ} (t : Expr cl 𝕄 γ) → [ σ ˢ∘ʳ ρ ]ˢ t  ≈ [ σ ]ˢ [ ρ ]ʳ t
    ...
    [ʳ∘ˢ]ˢ : ∀ {𝕄 cl} {γ δ η} → {σ : 𝕄 ∥ γ →ˢ δ} → {ρ : 𝕊 % δ →ʳ η} (t : Expr cl 𝕄 γ) → [ ρ ʳ∘ˢ σ ]ˢ t ≈ [ ρ ]ʳ ([ σ ]ˢ  t)
